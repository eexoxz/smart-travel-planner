...
The endpoint `GET /api/v1/planner/trips/:id/summary` demonstrates third-party API integration. It first reads the authenticated user's saved trip from the self-developed API, then sends the trip destination to Open-Meteo's geocoding API to obtain latitude and longitude. These coordinates are then used with Open-Meteo's forecast API to fetch current weather conditions and with OpenStreetMap Overpass API to fetch nearby places such as cafes, restaurants, attractions, museums, parks, beaches and shops. If the radius-based place search returns no usable results, the backend falls back to OpenStreetMap Nominatim using the saved destination, region and country text. For known demonstration cities, a curated fallback guide is also used when live place lookup is unavailable, so users still receive actual location recommendations instead of an empty plan.
